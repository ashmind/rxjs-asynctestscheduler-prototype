...
//import { hot, cold, expectObservable, expectSubscriptions, time } from 'rxjs/helpers/marble-testing';
...
// Taken from RXJS spec for TestScheduler. Incomplete.
...
describe('AsyncTestScheduler', () => {
  ...
  describe('parseMarbles()', () => {
    it('should parse a marble string into a series of notifications and types', () => {
      const result = AsyncTestScheduler.parseMarbles('-------a---b---|', { a: 'A', b: 'B' });
    ...
    it('should parse a marble string with an error', () => {
      const result = AsyncTestScheduler.parseMarbles('-------a---b---#', { a: 'A', b: 'B' }, 'omg error!');
      expect(result).deep.equal([
        { frame: 70, notification: Notification.createNext('A') },
        { frame: 110, notification: Notification.createNext('B') },
    ...
    it('should default in the letter for the value if no value hash was passed', () => {
      const result = AsyncTestScheduler.parseMarbles('--a--b--c--');
      expect(result).deep.equal([
        { frame: 20, notification: Notification.createNext('a') },
        { frame: 50, notification: Notification.createNext('b') },
    ...
    it('should handle grouped values', () => {
      const result = AsyncTestScheduler.parseMarbles('---(abc)---');
      expect(result).deep.equal([
    ...
    it('should ignore whitespace when runMode=true', () => {
      const runMode = true;
      const result = AsyncTestScheduler.parseMarbles('  -a - b -    c |       ', { a: 'A', b: 'B', c: 'C' }, undefined, undefined, runMode);
      expect(result).deep.equal([
    ...
    it('should suppport time progression syntax when runMode=true', () => {
      const runMode = true;
      const result = AsyncTestScheduler.parseMarbles('10.2ms a 1.2s b 1m c|', { a: 'A', b: 'B', c: 'C' }, undefined, undefined, runMode);
      expect(result).deep.equal([
  ...
  describe('parseMarblesAsSubscriptions()', () => {
    it('should parse a subscription marble string into a subscriptionLog', () => {
      const result = AsyncTestScheduler.parseMarblesAsSubscriptions('---^---!-');
    ...
    it('should ignore whitespace when runMode=true', () => {
      const runMode = true;
      const result = AsyncTestScheduler.parseMarblesAsSubscriptions('  - -  - -  ^ -   - !  -- -      ', runMode);
    ...
    it('should suppport time progression syntax when runMode=true', () => {
      const runMode = true;
      const result = AsyncTestScheduler.parseMarblesAsSubscriptions('10.2ms ^ 1.2s - 1m !', runMode);
  ...
  describe('createTime()', () => {
    it('should parse a simple time marble string to a number', () => {
      const scheduler = new AsyncTestScheduler(assertStub);
  ...
  */
  ...
  describe('AsyncTestScheduler.runAsync()', () => {
    ...
    describe('marble diagrams', () => {
      it('should ignore whitespace', async () => {
        ...
        await testScheduler.runAsync(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      it('should support time progression syntax', async () => {
        ...
        await testScheduler.runAsync(({ cold, hot, flush, expectObservable, expectSubscriptions }) => {
    ...
    it('should support explicit flushing', async () => {
      ...
      await testScheduler.runAsync(({ cold, expectObservable, flush }) => {
        const output = cold('-a-b-c|').pipe(
          concatMap(d => of(d).pipe(
            delay(10)
          ))
    ...
    it('should pass-through return values, e.g. Promises', async (done) => {
      const testScheduler = new AsyncTestScheduler(assertDeepEquals);

      await testScheduler.runAsync(() => {
